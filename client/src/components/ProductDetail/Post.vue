<template>
    <div class="d-flex flex-column flex-wrap">
        ...
        <div v-if="post.type == 'review'" class="d-flex flex-row flex-wrap ml-n3">
            <b-form-rating class="rating" :class="{ 'dark': darkMode }" variant="warning" inline no-border
                :value="post.review" readonly></b-form-rating>
        </div>
        <p class="align-self-start"> {{ post.description }} </p>
        <small class="ml-auto">Posted on {{ new Date(post.posted_date).toDateString() }}</small>
        <b-button v-if="post.type == 'discussion'" class="ml-auto" variant="success" size="sm"
            @click="replyModal = true">
            Reply
        </b-button>

        <b-modal v-if="post.type == 'discussion'" title="Reply discussion" v-model="replyModal" centered ok-title="Post"
            ok-variant="success" @ok="replyPost" cancel-title="Discard">
            <b-form-textarea v-model="replyMessage" placeholder="What's in your mind ?">
            </b-form-textarea>
        </b-modal>
        <hr>
    </div>
